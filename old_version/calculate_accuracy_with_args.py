...
parser.add_argument('--wgan_type', default = 'gp')
...
#PARAMETERS
MODE = 'wgan-gp' if args.wgan_type == 'gp' else 'wgan'#wgan_type#'wgan-gp' # wgan or wgan-gp
...
DATA_DIM = 32
LATENT_DIM = args.latent_dim#4
INITIALIZATION = args.weight_init#'glorot'#'he'#
...
with tf.Session() as session:
    ...
    for iteration in range(ITERS):
        ...
        for i in range(CRITIC_ITERS):
            _data = next(gen)
            _disc_cost, _, accuracy, fake_sample = session.run(
        ...
        lib.plot.plot('sample', fake_sample)
